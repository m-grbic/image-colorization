...
def test_loop(
        ...
        metric_calculator: MetricClaculator
    ):
...
def main():
    ...
    for temperature in config.temperatures:
        print(f"Calculating metrics for temperature {temperature}")
        image_colorizer = ImageColorizer(model=model, approach=config.model.approach, t=temperature)
        metrics_df = test_loop(
            test_dl=test_dl,
            image_colorizer=image_colorizer,
            metric_calculator=metric_calculator
        )
        meterics_dfs_list.append(metrics_df)
        metric_calculator.reset()
    
    total_metrics_df = pd.concat(meterics_dfs_list, ignore_index=True)
    total_metrics_df["temperature"] = config.temperatures
