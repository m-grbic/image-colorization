...
class ImageColorizerSE(nn.Module):
    ...
    def __init__(self, backbone_name: str, pretrained: bool = True, freeze_backbone: bool = False):
        super().__init__()

        # Dictionary to map backbone names to their respective models
        self.backbone_name = backbone_name.lower()
        backbone_dict = {
            'vgg': models.vgg16(weights='DEFAULT' if pretrained else None).features,
        ...
        # Upsampling layers
        self.upsample_to_64 = nn.Sequential(
            nn.Conv2d(512, 265, kernel_size=1, padding=1),
            nn.BatchNorm2d(265),
            nn.ReLU(inplace=True),
            nn.Upsample(size=(64, 64), mode='bilinear', align_corners=False),  # 32 -> 64
        )
        # For ResNet and ResNeXt, the output is typically (B, 2048, 7, 7), upsample it first to (B, 512, 32, 32)
    ...
    def forward(self, x):
        ...
        if self.backbone_name in ['resnet', 'resnext']:
            x = self.upsample_to_32(x)
        
        x = self.upsample_to_64(x)
