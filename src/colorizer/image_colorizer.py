...
class ImageColorizer:
    ...
    def __init__(self, model: torch.nn.Module, t: float = 0.38) -> None:
    ...
    def __call__(self, x: torch.Tensor):
        ...
        pred_prob = self._model(x)  # (BS, 256, 64, 64)
        ab_predictions = self._anealed_mean(pred_prob)  # (BS, 1, 64, 64)
    ...
    def reconstruct_image(self, l: np.ndarray, ab: torch.Tensor) -> None:
        ...
        ab_upsampled = F.interpolate(ab.unsqueeze(0), size=(224, 224), mode='bicubic', align_corners=False).squeeze(0) + 128

        image_lab = torch.cat((l_tensor, ab_upsampled), dim=0)
